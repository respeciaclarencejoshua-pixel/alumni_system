import { toLocalDateTime, toApiDateTime } from '../../lib/adminDates.js';
...
export default function OpportunitiesEvents({ scopes = [] }) {
  const canOpportunities = scopes.includes('opportunities');
  const canEvents = scopes.includes('events');
  ...
  async function load() {
    try { const [opportunitiesResult, eventsResult] = await Promise.all([canOpportunities ? adminApi('/api/admin/opportunities') : Promise.resolve({}), canEvents ? adminApi('/api/admin/resources/events') : Promise.resolve({})]); setOpportunities(opportunitiesResult.opportunities || []); setEvents((eventsResult.resources || []).map((item) => ({ id: item.id, ...item.payload, created_at: item.created_at }))); }
  ...
  async function saveEvent(event) { 
    ...
    try { 
      let payload = { ...eventForm, date: toApiDateTime(eventForm.date), endDate: toApiDateTime(eventForm.endDate), registrationDeadline: toApiDateTime(eventForm.registrationDeadline) };
      delete payload.id;
  ...
  const visible = opportunities.filter((item) => tab==='review'?reviewStatuses.includes(item.status):tab==='archived'?['archived','expired','rejected'].includes(item.status):['active','approved','published','paused','draft'].includes(item.status)), activeCount = opportunities.filter((item) => item.status === 'active').length, reviewCount=opportunities.filter(item=>reviewStatuses.includes(item.status)).length;
  return <div className="jobs-events-page admin-operations-page">
    <header className="admin-page-header jobs-events-header"><div><p>Operations</p><h1>Opportunities &amp; Events</h1><span>Review submissions, publish trusted opportunities, and manage the community calendar.</span></div>{canOpportunities && <div className="jobs-events-tabs"><button className={tab === 'review' ? 'active' : ''} onClick={() => setTab('review')}>Review ({reviewCount})</button><button className={tab === 'active' ? 'active' : ''} onClick={() => setTab('active')}>Published</button><button className={tab === 'archived' ? 'active' : ''} onClick={() => setTab('archived')}>History</button></div>}</header>
