...
export default function Chat({ user }) {
  ...
  async function uploadFile(event) {
    ...
    await insertMessage({ body: file.name, message_type: file.type.startsWith('image/') ? 'image' : 'file', attachment_url: path, attachment_name: file.name, attachment_type: file.type });
  }
  ...
  async function searchGifs(event) {
    ...
    if (!key) return setGifError('GIPHY API key is missing. Restart Vite after adding it.');
    setGifLoading(true); setGifError(''); setGifs([]);
    try {
      const response = await fetch(`https://api.giphy.com/v1/gifs/search?api_key=${encodeURIComponent(key)}&q=${encodeURIComponent(gifQuery.trim())}&limit=12&rating=g`);
      const json = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(json?.meta?.msg || `GIPHY rejected the request (${response.status}).`);
      setGifs(json.data || []);
      if (!json.data?.length) setGifError('No GIFs found. Try another search.');
    } catch (requestError) {
  ...
  async function loadTrendingGifs() {
    ...
    try {
      const response = await fetch(`https://api.giphy.com/v1/gifs/trending?api_key=${encodeURIComponent(key)}&limit=12&rating=g`);
      const json = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(json?.meta?.msg || `GIPHY rejected the request (${response.status}).`);
      setGifs(json.data || []);
      if (!json.data?.length) setGifError('Trending GIFs are unavailable right now.');
    } catch (requestError) {
      setGifError(requestError.message || 'Could not connect to GIPHY.');
    } finally { setGifLoading(false); }
  }
  ...
  return <div className="messenger-root">
    ...
    {selected && <section className={`messenger-window ${minimized ? 'minimized' : ''}`}>
      <header onClick={() => minimized && setMinimized(false)}><Avatar person={selected} small/><div><strong>{nameOf(selected)}</strong><small>Registered alumni</small></div><button onClick={(e) => { e.stopPropagation(); setMinimized(!minimized); }} aria-label="Minimize chat">−</button><button onClick={(e) => { e.stopPropagation(); setSelected(null); }} aria-label="Close conversation">×</button></header>
      {!minimized && <>
