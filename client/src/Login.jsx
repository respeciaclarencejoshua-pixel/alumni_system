...
export default function Login({ onRegister, onClose }) {
  ...
  async function submit(event) {
    ...
    if (captchaEnabled && !captchaToken) { setMessage('Complete the bot-protection check first.'); setLoading(false); return; }
    if (mode === 'recovery') {
      const { error } = await supabase.auth.resetPasswordForEmail(email.trim().toLowerCase(), { redirectTo: `${window.location.origin}/`, ...(captchaEnabled ? { captchaToken } : {}) });
